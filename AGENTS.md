...
- 通过 Host 全局余额快照变化估算每日费用；该费用仅为估算值，不等同于官方账单。

余额来自 DeepSeek 官方 `/user/balance` 接口；每日 Token 并无官方历史查询接口，因此只能从插件安装并启动后开始本地累计。

## 核心结构

- `lib/index.js`：Host 端。解析凭据、查询余额、监听 `llm/stream`、持久化每日 Token 与全局费用快照，并注册查询及历史导入路由。
...
- 不要假设 Host 连接 Context、React 组件和 Slot 注册一定同步卸载；`connection/reset` 必须通过稳定桥接事件通知仍存活的 Dashboard 立即刷新，并在失败时保留上次成功数据。
...
2. 确认 `/api/deepseek-balance` 返回 `ok: true`、`schemaVersion: 4`、`costDaily`，且响应不包含明文 API Key。
3. 验证 `/api/deepseek-balance/history` 对 localStorage 历史幂等导入、限制请求体，并在重复导入时不重复计算费用。
4. 验证插件 stop/重载时两个 API 路由会随 Context 正确注销，不发生重复注册。
