  ...
  factory: function (require) {
    ...
    exports.apply = function (ctx) {
      ...
      function Dashboard() {
        ...
        var backgroundState = React.useState(false)
        var backgroundRefreshing = backgroundState[0]
        var setBackgroundRefreshing = backgroundState[1]
        ...
        function fetchBalance(options) {
          ...
          var isBackground = options.background === true
          if (isBackground) {
          ...
          if (!options.silentFailure) setError('')
          var url = API_URL
          if (options.cached) url += '?cached=1'
          else if (options.force) url += '?force=1'
          return fetch(url).then(function (resp) { return resp.json() }).then(function (res) {
            if (requestId !== requestIdRef.current) return false
            if (isBackground) setBackgroundRefreshing(false)
            ...
            if (!options.silentFailure) showFetchFailure((res && res.error) ? res.error : '查询失败')
            return false
          }).catch(function (e) {
            if (requestId !== requestIdRef.current) return false
            if (isBackground) setBackgroundRefreshing(false)
            else setLoading(false)
            if (!options.silentFailure) showFetchFailure(e && e.message ? e.message : String(e))
            return false
          })
        }

        function refreshCachedThenLive() {
          return fetchBalance({ cached: true, silentFailure: true }).then(function (cacheHit) {
            return fetchBalance({ background: cacheHit })
          })
        }
        function refreshManual() { return fetchBalance({ force: true }) }
        function refreshInBackground() { return fetchBalance({ background: hasGoodDataRef.current }) }

        React.useEffect(function () {
          refreshCachedThenLive()
          var disposeInterval = ctx.interval(refreshInBackground, AUTO_MS)
          window.addEventListener(REFRESH_EVENT, refreshCachedThenLive)
          return function () {
            window.removeEventListener(REFRESH_EVENT, refreshCachedThenLive)
          ...
          React.createElement('div', { className: 'dsb-page-head' },
            React.createElement('div', null,
              ...
              React.createElement('div', { className: 'dsb-hint' },
                balance
                  ? ('密钥来源：' + sourceLabel(balance.keySource) + ' · 每 5 分钟自动刷新'
                    + (backgroundRefreshing ? ' · 后台更新中…' : (balance.cached ? ' · 已显示缓存' : ''))
                    + (legacyCount ? ' · 含 ' + legacyCount + ' 天历史估算记录' : ''))
                  : (loading ? '正在读取全局缓存…' : '密钥从 Harness 凭据自动读取，无需手动输入')
              )
            ),
            React.createElement('button', { className: 'dsb-btn', disabled: loading, onClick: refreshManual }, loading ? '查询中…' : '↻ 刷新')
          ),
