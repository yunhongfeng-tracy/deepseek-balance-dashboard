...
export function apply(ctx) {
  ...
  async function discoverLegacyFiles() {
    const paths = new Set([
      resolve(cwd, LEGACY_TOKEN_FILE),
      resolve(cwd, LEGACY_TOKEN_FILE_OLD),
      resolve(dshHome, LEGACY_TOKEN_FILE),
      resolve(dshHome, LEGACY_TOKEN_FILE_OLD)
    ])
    try {
  ...
  // 路由生命周期绑定插件 Context：插件 stop/重载时自动移除，避免泄漏或重复注册。
