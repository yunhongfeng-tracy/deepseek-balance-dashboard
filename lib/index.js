...
export function apply(ctx) {
  ...
  async function loadTokenDaily() {
    ...
    try {
      ...
      const daily = obj && obj.version === TOKEN_SCHEMA_VERSION && obj.provider === DEEPSEEK_PROVIDER
        && obj.daily && typeof obj.daily === 'object' && !Array.isArray(obj.daily)
        ? obj.daily
        : null
      if (daily) {
        for (const k in daily) {
          if (!tokenDaily[k] && daily[k] && typeof daily[k].total === 'number') tokenDaily[k] = daily[k]
        }
      } else {
        // v1 曾把所有 provider 都算进 DeepSeek，并重复计入 reasoningTokens；
        // 旧数据无法可靠拆分，升级时清空后从正确口径重新累计。
        persistTokenDaily()
      }
    } catch (e) { /* 文件不存在或损坏时忽略 */ }
  }
  ...
  function persistTokenDaily() {
    ...
    const payload = { version: TOKEN_SCHEMA_VERSION, provider: DEEPSEEK_PROVIDER, daily: snap }
    fs.writeText(tokenTarget, JSON.stringify(payload)).catch(function () {})
  }
